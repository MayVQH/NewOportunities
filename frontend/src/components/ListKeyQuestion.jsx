...
const ListKeyQuestion = () => {
    ...
    useEffect(() => {
            const fetchKeyQuestion = async () => {
                try {
                    ...
                    const response = await fetch(`${import.meta.env.VITE_BACKEND_ORIGIN}/api/themes/preguntasClave/usuario/${user.id}`);
                    if (!response.ok) {
    ...
    return (
            <div className="d-flex flex-column min-vh-100">
                ...
                {(user.tipoId === import.meta.env.VITE_MODER_UID || user.tipoId === import.meta.env.VITE_COM_UID) && (
