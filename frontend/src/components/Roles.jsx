...
const PruebaRoles = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_BACKEND_ORIGIN}/api/themes/roles`);
        if (!response.ok) {
  ...
  const tipos = [ 
    {id: import.meta.env.VITE_COM_UID,nombre:	'comite'},
    {id: import.meta.env.VITE_MODER_UID,nombre:	'moderador'},
    {id:import.meta.env.VITE_USER_UID,nombre:'usuario'}]
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_ORIGIN}/api/themes/updatedroles/${idUsuario}/${id}`,{method:'PUT'});
      if (!response.ok) {
