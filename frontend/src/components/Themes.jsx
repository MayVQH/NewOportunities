...
const Temas = () => {
    ...
    useEffect(() => {
        const fetchThemesId = async () => {
            try {
                const response = await fetch(`${import.meta.env.VITE_BACKEND_ORIGIN}/api/themes/totales/preguntasTemas`);
                if (!response.ok) {
                    const errorData = await response.json().catch(() => ({
                        message: 'Error desconocido'
                    }));
    ...
    const confirmDelete = async () => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_ORIGIN}/api/themes/${themeToDelete}`, {
                method: 'DELETE'
            });
            if (!response.ok) throw new Error('Error al eliminar el tema');

            const updatedResponse = await fetch(`${import.meta.env.VITE_BACKEND_ORIGIN}/api/themes`);
